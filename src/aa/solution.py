    ...
    def openLock(self, deadends, target):
        ...
        while len(bfs) > 0:
            ...
            for st in range(size):
                foo = bfs.pop(0)
                next_strs = self._forward(foo)
                for chl in next_strs:
                    if chl not in can_not_use.keys():
                        if chl == target:
                            return cnt
                        bfs.append(chl)
                        can_not_use[chl] = True
            cnt += 1
        return -1

    def _to_string(self, chl):
        return ''.join(str(i) for i in chl)

    def _forward(self, foo):
        res = []
        li = [int(i) for i in foo]
        for l in range(len(li)):
            tmp = li[l]
            li[l] = (tmp + 1) % 10
            res.append(self._to_string(li))
            li[l] = (tmp - 1) % 10
            res.append(self._to_string(li))
            li[l] = tmp
        return res
...
if __name__ == '__main__':
    ss = Solution()
    a = ["8887", "8889", "8878", "8898", "8788", "8988", "7888", "9888"]
    b = '8888'
    print(ss.openLock(a, b))
